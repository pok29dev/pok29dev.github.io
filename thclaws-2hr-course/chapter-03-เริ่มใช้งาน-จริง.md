...
> 📷 **รูปที่ 3-2 — ทัวร์ Desktop GUI (มีตัวเลขชี้จุด)** *(screenshot: ภาพหน้าจอรวมของ thClaws พร้อม callout ตัวเลขชี้ Chat / Settings / Sessions / Files / Permission)*
...
> 📷 **รูปที่ 3-3 — หน้าจอ Sessions และปุ่ม New session** *(screenshot: รายการ session + ตำแหน่งปุ่ม New session)*
...
> 📷 **รูปที่ 3-4 — ตัวอย่างคำสั่งแรกใน Chat tab และคำตอบ** *(screenshot: พิมพ์คำสั่งแล้วเห็นคำตอบจาก Agent)*

สังเกต: Agent ใช้ instruction ที่คุณตั้งในบทที่ 2 + สำรวจ working directory ก่อนตอบ

### 5.2 ส่งคำสั่งแรกผ่าน One-shot (ทำงานผ่าน terminal)
...
```bash
คำสั่งนี้คือการทำงานผ่าน terminal
...
เปิด thClaws (GUI) ที่ Chat UI แล้วลองตามลำดับนี้ (ใช้ไฟล์ `meeting_notes.txt` ที่คัดลอกไว้):
...
thClaws **จะขออนุญาตก่อน** ที่จะทำงานที่ไวต่อผล เช่น สร้างไฟล์, รันคำสั่ง, เข้าเน็ต — แต่คุณควบคุมได้ว่าจะให้ขอเมื่อไหร่ (ค่าเริ่มต้นจะเป็น auto) ด้วยคำสั่ง:
...
| `linegated` | ส่งคำขออนุมัติไปที่ **LINE chat** — เปิดใช้งานอัตโนมัติเมื่อจับคู่ LINE แล้ว (ดูคู่มือบทที่ 21) |
...
- [ ] เดินชม GUI ครบจุด (Chat / Settings / Sessions / Files) (หัวข้อ 3)
...
> 💡 สังเกตตอนสั่งสร้างไฟล์ — Agent จะขอ permission ให้คุณกดอนุญาต (จะไม่ขึ้นถ้าตั้งค่า auto ไว้) ตรวจดูสิ่งที่มันจะทำก่อนเสมอ
...
| "เผลอปิด session จะหายไหม" | ไม่หาย — ประวัติเก็บใน list หรือไฟล์ใน `./.thclaws/sessions/` กลับมาเปิดต่อได้ |
