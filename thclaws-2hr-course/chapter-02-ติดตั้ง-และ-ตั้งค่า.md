...
![รูปที่ 2-5 — Chat UI คำตอบของ Agent](screenshots/ss-2-5.png)

> 📷 **รูปที่ 2-5 — ผลลัพธ์จากคำสั่งทดสอบ** *(screenshot: Chat UI คำตอบของ Agent)*

- ✅ ได้คำตอบ = พร้อมใช้ → ขึ้นบท 3 ได้เลย
- ❌ มี error → ดู หัวข้อ 9 Troubleshooting

---

## 8. ตั้งค่าเพิ่มเติม: เลือกโมเดล + instruction + ฟีเจอร์เสริม

### 8.1 เลือกโมเดลตามความสามารถ 

> ✅ **หลักสำคัญ:** การเลือก model ขึ้นกับ **คุณสมบัติ/ความสามารถ** ของแต่ละตัวว่ารองรับงานแบบไหน
...
> ก่อนเลือกโมเดล ให้ถามตัวเองว่า **"งานนี้ต้องการโมเดลที่เก่งด้านไหน?"** — เร็ว/คุ้มค่า คุณภาพสูง หรือรองรับภาพ วิดีโอ
...
- [ ] ตรวจความพร้อมเครื่อง (หัวข้อ 3): ระบบ/พื้นที่/เน็ต/สิทธิ์
- [ ] ติดตั้ง thClaws สำเร็จ (อย่างน้อย 1 วิธีตาม หัวข้อ 4)
- [ ] `thclaws --version` ทำงาน
- [ ] สร้างโฟลเดอร์ `~/thclaws-2hr` (หัวข้อ 5)
- [ ] สร้าง Qwen API Key ที่ DashScope (หัวข้อ 6) — Key ขึ้นต้น `sk-…`
- [ ] ใส่ Key ผ่าน GUI หรือ CLI (หัวข้อ 7)
- [ ] พิมพ์ข้อความ "สวัสดี ช่วยแนะนำตัวหน่อย" แล้ว**ได้คำตอบ** ✅ 
- [ ] ตั้ง instruction ไฟล์แรก `AGENTS.md` (หัวข้อ 8.2)
...
| อยากทำงาน offline | ไม่อยากใช้ Key / ข้อมูลไม่ต้องส่งออกนอกบริษัท | ใช้ Local Model — รันในเครื่องไม่ใช้เน็ต |

**คำถามที่คนเริ่มต้นมักถาม:**
| คำถาม | คำตอบ |
|--------|-------|
| "ใช้เครื่องบริษัทที่มี Policy ติดตั้งไม่ได้" | ใช้วิธี Docker/portable ติดตั้งจาก zip files หรือขอยกเว้นจากไอที — อย่าบังคับติดตั้ง |
| "ไม่มีบัญชี Alibaba Cloud" | สมัครฟรีที่ DashScope; ใช้บัญชี Aliyun login ได้ หรือติดต่อจากไอทีซัพพอร์ตของบริษัท |
| "Key `401/403` ทำไง" | เช็ก `sk-…` คัดลอกครบ + activate โมเดลในหน้า Model + เช็ค credit ที่ได้รับ |
| "เลือกโมเดลไหนดี" | งานทั่วไป `qwen-plus`, งานไว `qwen-turbo`, งานละเอียด `qwen-max` — ตามความสามารถ |
